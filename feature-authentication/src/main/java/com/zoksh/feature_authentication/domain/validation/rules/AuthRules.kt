...
class InvalidEmailHandler(private val email: String) : ValidationHandler() {
    ...
    override fun validate() = if (!emailRegex.matches(email))
...
class PasswordTooShortHandler(private val password: String, private val minLength: Int = 8) : ValidationHandler() {
    override fun validate() = if (password.length < minLength)
        ValidationError.PasswordTooShort
    else null
}

class PasswordNoUpperCaseHandler(private val password: String) : ValidationHandler() {
    override fun validate() = if (password.none { it.isUpperCase() })
        ValidationError.PasswordNoUpperCase
    else null
}

class PasswordNoLowerCaseHandler(private val password: String) : ValidationHandler() {
    override fun validate() = if (password.none { it.isLowerCase() })
        ValidationError.PasswordNoLowerCase
    else null
}

class PasswordNoNumberHandler(private val password: String) : ValidationHandler() {
    override fun validate() = if (password.none { it.isDigit() })
        ValidationError.PasswordNoNumber
    else null
}

class PasswordNoSpecialCharHandler(private val password: String) : ValidationHandler() {
    override fun validate() = if (password.none { !it.isLetterOrDigit() })
        ValidationError.PasswordNoSpecialChar
    else null
}

class PasswordNoMatchingHandler(private val password: String, private val confirmPassword: String) : ValidationHandler() {
    override fun validate() = if (password != confirmPassword)
        ValidationError.PasswordNoMatching
    else null
}
